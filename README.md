...
- `npm test` regenera `index.html` y falla si no coincidía con el del repo (para no publicar una versión desactualizada), comprueba que los datos de hombro coinciden con `tarjeta_hombro.js`, que los campos de `spa_hombro.js` nombran filas que existen y que el texto en pantalla coincide con el de la tarjeta
