...
SPA móvil de solo lectura con tarjetas de consulta de fisioterapia (Banderas rojas, Bisagra y árbol, fichas de síndrome). El inicio es un selector de región; hoy hombro, cadera y lumbar tienen datos, cervical y rodilla se ven pero están pendientes. No guarda datos.
...
- `npm run build` junta los `<región>.data.json` que existan (hoy hombro, cadera y lumbar) y genera `index.html` a partir de `plantilla.html`
...
- `npm test` regenera `index.html` y falla si no coincidía con el del repo (para no publicar una versión desactualizada), comprueba que los datos de hombro, cadera y lumbar coinciden con su `tarjeta_<región>.js`, que los campos de `spa_<región>.js` nombran filas que existen, que el texto en pantalla coincide con el de cada tarjeta, el selector de región del inicio, y que `package.json` e `index.html` coinciden con el nombre de `app.json`
