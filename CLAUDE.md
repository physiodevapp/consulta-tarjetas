...
- `build.py` lee `app.json` y junta los `<región>.data.json` que existan (define las cinco regiones conocidas; hoy `hombro.data.json` y `cadera.data.json`) en `{APP, REGIONES: [{region, nombre, disponible}], DATOS: {<región>: ...}}`, lo inyecta en la plantilla y genera `index.html` (lo que publica GitHub Pages). Solo biblioteca estándar
