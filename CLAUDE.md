...
- `spa_<región>.js` **los campos específicos de la SPA viven aquí, no en el CONTENIDO de la tarjeta**: enlaces del árbol a las fichas, correspondencia síndrome↔pronóstico, pistas de las fichas, agrupación de la tabla orientativa y corte de la nota. Hoy `spa_hombro.js` y `spa_cadera.js`. No es texto clínico: solo nombra filas que ya están en la tarjeta. `ENLACES[nodo].botones` es la forma general (destinos fijos); `.patrones` (solo hombro) es un atajo para cuando el nodo reparte un patrón limpio en una sola línea — no lo fuerces si no encaja (ver «Lo específico de cadera»)
...
- `test-contenido.js` (hombro), `test-contenido-cadera.js`, `test-recorrido.js`, `test-cabecera.js`, `test-urgencia.js`, `test-selector.js` pruebas con jsdom. Al añadir una región, prueba de contenido propia (como `test-contenido-cadera.js`), no ampliar la de hombro
...
3. ~~**Selector de región en el inicio.**~~ **Hecho.** `pSelector()` en `plantilla.html`, con `DATA.REGIONES` (las cinco, generadas por `build.py`): hombro y cadera abren, las otras tres se ven marcadas «Pendiente» y no son clicables. El nombre de la región se ve siempre en la cabecera (`#regionCab`) salvo en el propio selector y en el inicio de la región (ahí ya es el título); también con las cápsulas del recorrido. Probado en `test-selector.js` y en `test-cabecera.js`.
4. **Añadir regiones una a una** (cervical, lumbar, rodilla; cadera hecha) leyendo su `tarjeta_<región>.js` con `node extraer.js tarjeta_<región>.js` y dándole su `spa_<región>.js`. Para cada una, prueba de contenido propia (`test-contenido-cadera.js` como referencia): el texto en pantalla coincide con el de su tarjeta, y **todas** las fichas de síndrome y la tabla orientativa alcanzables desde el árbol (decisión 3). Tener en cuenta las diferencias de arriba: no asumir los nodos de hombro (`botones` cubre lo que `patrones` no encaja), tratar `span`, `BANDERAS` de 2 columnas (lumbar) y pronóstico con mapa explícito (`PRONOSTICO_DE`, con `null` si a una síndrome no le toca fila).
