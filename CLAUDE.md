...
- `test-contenido.js` (hombro), `test-contenido-cadera.js`, `test-contenido-lumbar.js`, `test-recorrido.js`, `test-cabecera.js`, `test-urgencia.js`, `test-selector.js` pruebas con jsdom. Al añadir una región, prueba de contenido propia (como las de cadera o lumbar), no ampliar la de hombro
...
3. ~~**Selector de región en el inicio.**~~ **Hecho.** `pSelector()` en `plantilla.html`, con `DATA.REGIONES` (las cinco, generadas por `build.py`): hombro, cadera y lumbar abren, cervical y rodilla se ven marcadas «Pendiente» y no son clicables. El nombre de la región se ve siempre en la cabecera (`#regionCab`) salvo en el propio selector y en el inicio de la región (ahí ya es el título); también con las cápsulas del recorrido. Probado en `test-selector.js` y en `test-cabecera.js`.
4. **Añadir regiones una a una** (cervical, rodilla; cadera y lumbar hechas) leyendo su `tarjeta_<región>.js` con `node extraer.js tarjeta_<región>.js` y dándole su `spa_<región>.js`. Para cada una, prueba de contenido propia (`test-contenido-cadera.js` o `test-contenido-lumbar.js` como referencia): el texto en pantalla coincide con el de su tarjeta, y **todas** las fichas de síndrome y la tabla orientativa alcanzables desde el árbol (decisión 3). Tener en cuenta las diferencias de arriba: no asumir los nodos de hombro (`botones` cubre lo que `patrones` no encaja; se pueden combinar en el mismo nodo), tratar `span`, la matriz comparativa de `ORIENTATIVA` (rodilla y cervical tienen formas propias, no asumir la de lumbar/cadera) y pronóstico con mapa explícito (`PRONOSTICO_DE`, con `null` si a una síndrome no le toca fila; no inventar el destino si la tarjeta no lo dice, como con «Curso general» en lumbar).
